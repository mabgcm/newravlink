...
function RecordingActions({ record }) {
  const [copied, setCopied] = useState(false);
  const copyLink = async () => {
    await navigator.clipboard.writeText(record.recordingUrl);
    setCopied(true);
    window.setTimeout(() => setCopied(false), 2000);
  };
  return <div className="record-actions"><a className="record-icon-button open" href={record.recordingUrl} target="_blank" rel="noreferrer" aria-label="Play recording" title="Play recording"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5.5v13l10-6.5-10-6.5Z" /></svg></a><button className={`record-icon-button copy ${copied ? "copied" : ""}`} type="button" onClick={copyLink} aria-label={copied ? "Recording link copied" : "Copy recording link"} title={copied ? "Copied" : "Copy link"}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 8.5h9.5V18H9z" /><path d="M5.5 15.5V5.5h10" /></svg></button></div>;
}
...
function CallerDashboard({ user, onLogout }) {
  ...
  const startCall = async () => {
    ...
    try {
      ...
      const call = await deviceRef.current.connect({ params: { To: number.replace(/\s/g, ""), CallId: callId, Agent: user.username || user.sub } });
  ...
  return <main className="caller-shell"><header className="caller-top"><div><span className="caller-mark small"><img src="/assets/images/ravlink-caller-icon.png" alt="" /></span><strong>Ravlink Caller</strong></div><div><span>{user.name || user.username}</span><button className="text-button" onClick={onLogout}>Sign out</button></div></header><div className="caller-grid"><section className="caller-card dialer"><p className="eyebrow">NEW CALL</p><h1>{active ? (callLabels[status] || "Connecting") : "Who would you like to call?"}</h1><label className="phone-field"><span>Phone number</span><input inputMode="tel" value={number} onChange={(e) => setNumber(e.target.value)} disabled={active} placeholder="+1 416 555 1234" /></label>{active && <div className="call-live"><span className="pulse" />{callLabels[status] || "Connecting"}<strong>{time}</strong></div>}{message && <div className="caller-message">{message}</div>}<div className="call-actions">{!active ? <button className="primary call-button" onClick={startCall}><span>☎</span> Start call</button> : <><button className={`round ${muted ? "selected" : ""}`} onClick={toggleMute} aria-label={muted ? "Unmute" : "Mute"}>{muted ? "🔇" : "🎙"}</button><button className="danger call-button" onClick={endCall}>End call</button></>}</div></section><section className="caller-card history"><div className="section-title"><div><p className="eyebrow">CALL HISTORY</p><h2>Recent recordings</h2></div><button className="icon-button" onClick={loadRecords} aria-label="Refresh recordings">↻</button></div>{records.length === 0 ? <div className="empty"><span>◉</span><p>No recordings yet.</p></div> : <div className="record-list">{records.map((record) => <article key={record.callId}><div className="record-details"><strong>{record.to}</strong><time dateTime={record.completedAt || record.updatedAt}>{formatDate(record.completedAt || record.updatedAt)}</time></div>{record.recordingUrl ? <RecordingActions record={record} /> : <span className="processing">Processing…</span>}</article>)}</div>}</section></div></main>;
}
