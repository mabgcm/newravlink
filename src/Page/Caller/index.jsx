...
function RecordingActions({ record }) {
  ...
  return <div className="record-actions"><span className="record-link-preview" title={record.recordingUrl}>Recording link</span><div><a className="record-action open" href={record.recordingUrl} target="_blank" rel="noreferrer">Play ↗</a><button className={`record-action copy ${copied ? "copied" : ""}`} type="button" onClick={copyLink}>{copied ? "Copied ✓" : "Copy link"}</button></div></div>;
}
