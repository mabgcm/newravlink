...
export default async function handler(req, res) {
  ...
  if (callId) await db().collection("callRecords").doc(callId).set({ callId, to: req.query.to, agent: req.query.agent, callSid: body.CallSid || null, status: body.CallStatus || "unknown", updatedAt: new Date().toISOString() }, { merge: true });
  res.status(204).end();
}
